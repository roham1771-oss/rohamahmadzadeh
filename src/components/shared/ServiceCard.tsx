...
const iconMap: Record<string, JSX.Element> = {
  ...
  Landmark: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 14v3m4-3v3m4-3v3M3 21h18M3 10h18M3 7l9-4 9 4M4 10h16v11H4V10z" /></svg>,
...
export function ServiceCard({ service, locale }: ServiceCardProps) {
  ...
  const href = locale === 'fa' ? `/services/${service.slug}` : `/en/services/${service.slug}`;
  const icon = service.icon && iconMap[service.icon] ? iconMap[service.icon] : iconMap.Scale;

  return (
    <Link href={href} className="card group p-8 hover-lift block">
      <div className="w-14 h-14 bg-accent-500/10 rounded-2xl flex items-center justify-center mb-6 text-accent-500 group-hover:bg-accent-500 group-hover:text-white transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-accent-500/25">
        {icon}
      </div>
      <h3 className="text-xl font-bold mb-3 group-hover:text-accent-600 dark:group-hover:text-accent-400 transition-colors">
        {title}
      </h3>
      <p className="text-muted-foreground text-sm leading-relaxed line-clamp-3">
        {description}
      </p>
      <div className="mt-5 text-sm font-medium text-accent-600 dark:text-accent-400 flex items-center gap-2 group-hover:gap-3 transition-all duration-300">
        {locale === 'fa' ? 'مشاهده جزئیات' : 'View Details'}
        <svg className={`w-4 h-4 transition-transform duration-300 group-hover:translate-x-1 ${locale === 'fa' ? 'rotate-180 group-hover:-translate-x-1' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
